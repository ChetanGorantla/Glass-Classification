...
def calculate_emi(p, n, r):
  ...
  st.write("Estimated", emi)
def calculate_outstanding_balance(p, n, r, m):
  balance = (p * ((1+(r/100))**n) - ((1+(r/100))**m))/(((1+(r/100)**n)-1))
  st.write("Balance", balance)
st.title("EMI & Balance Calculator")
principal = st.slider("Principal 1", 1000,10000)
tenure = st.slider("Tenure 1", 1000, 10000)
roi = st.slider("Rate of Interest", 1, 15)
period = st.slider("Period after which the Outstanding Loan Balance is calculated (in months)", 1, tenure*12)
n = tenure * 12
r = roi/12
if st.button("Run"):
  calculate_emi(principal, n, r)
elif st.button("Balance Checking"):
